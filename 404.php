...
<head>
    ...
    <title itemprop="name">
        ...
        if ($paged >= 2 || $page >= 2)
            echo ' - ' . sprintf(__('page %s'), max($paged, $page));/*第 %s 页*/ ?>
    </title>
...
<body <?php body_class(); ?>>
    <section class="error-404 not-found">
        ...
        <div class="err-button back">
            ...
            <a id="gohome" href="<?php bloginfo('url'); ?>">
                <?php _e('return to home page', 'sakura');/*返回主页*/ ?>
            </a>
        </div>
        <div style="display:block; width:284px;margin: auto;">
            <p style="margin-bottom: 1em;margin-top: 1.5em;text-align: center;font-size: 15px;">
                <?php _e('Don\'t worry, search in site?', 'sakura');/*别急，试试站内搜索？*/ ?>
            </p>
            <form class="s-search" method="get" action="/" role="search">
                ...
                <input class="text-input" style="padding: 8px 20px 8px 46px;" type="search" name="s"
                    placeholder="<?php _e('Search...', 'akina') ?>" required>
            </form>
        </div>
    </section>
